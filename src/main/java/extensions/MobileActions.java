...
public class MobileActions extends CommonOps{
    ...
    @Step("Drag and Drop")
    public static void dragNDrop(MobileElement start, MobileElement finish){
        TouchAction action = new TouchAction(mobileDriver);
        action.press(new ElementOption().withElement(start)).moveTo(new ElementOption().withElement(finish))
                .release().perform();
    }
